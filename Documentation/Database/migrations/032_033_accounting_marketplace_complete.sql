-- Complete Migration: Accounting Marketplace System
-- Description: สร้างตารางและเพิ่ม notification type สำหรับระบบตลาดกลางผู้ทำบัญชี
-- Created: 2026-02-04
...
SELECT 'Table accounting_marketplace_listings created successfully' AS status
WHERE EXISTS (
  SELECT 1 FROM information_schema.tables 
  WHERE table_schema = DATABASE() 
  AND table_name = 'accounting_marketplace_listings'
);

-- Verify notification type added
SELECT 'Notification type accounting_marketplace_sold added successfully' AS status
WHERE EXISTS (
  SELECT 1 FROM information_schema.columns 
  WHERE table_schema = DATABASE() 
  AND table_name = 'notifications' 
  AND column_name = 'type'
  AND column_type LIKE '%accounting_marketplace_sold%'
);
