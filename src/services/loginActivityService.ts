...
export const loginActivityService = {
  getStats: async (): Promise<LoginStats> => {
    const response = await api.get<{ success: boolean; data: LoginStats }>(
  ...
  getAttempts: async (params: {
    ...
    }>("/login-activity/attempts", { params });
    return response.data.data;
  },
  ...
  getChartData: async (days: number = 7): Promise<ChartDataPoint[]> => {
    const response = await api.get<{
      success: boolean;
      data: ChartDataPoint[];
    }>("/login-activity/chart", { params: { days } });
    return response.data.data;
  },
  ...
  sendHeartbeat: async (
    ...
    }>("/login-activity/heartbeat", { sessionId });
  ...
  deleteAttempts: async (ids: string[]): Promise<{ deletedCount: number }> => {
    const response = await api.delete<{
      success: boolean;
      deletedCount: number;
    }>("/login-activity/attempts", { data: { ids } });
  ...
  deleteAllAttempts: async (
    ...
    const response = await api.delete<{
      success: boolean;
      deletedCount: number;
    }>("/login-activity/attempts", { data: { deleteAll: true, beforeDate } });
    return { deletedCount: response.data.deletedCount };
  },

  getExternalIpAttempts: async (
    today?: boolean,
  ): Promise<{ attempts: LoginAttempt[]; count: number }> => {
