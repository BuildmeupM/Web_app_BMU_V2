...
export const companyFeedService = {
  ...
  deleteComment: async (postId: string, commentId: string) => {
    const res = await api.delete(
      `/company-feed/posts/${postId}/comments/${commentId}`,
    );
    return res.data;
  },
  ...
  // ── Acknowledgements ──
  acknowledgePost: async (postId: string) => {
    const res = await api.post<{
      success: boolean;
      data: { is_acknowledged: boolean };
    }>(`/company-feed/posts/${postId}/acknowledge`);
    return res.data.data;
  },
