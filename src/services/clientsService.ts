...
const clientsService = {
  ...
  async getDropdownList(params?: {
    ...
    const response = await api.get<{
      success: boolean;
      data: { build: string; company_name: string }[];
    }>("/clients/dropdown", { params });
  ...
  async update(build: string, data: Partial<Client>): Promise<Client> {
    const response = await api.put<ClientDetailResponse>(
      `/clients/${build}`,
      data,
    );
    return response.data.data;
  ...
  async getStatistics(): Promise<{
    ...
    incompleteData: {
      basicInfo: {
        count: number;
        clients: Array<{ build: string; company_name: string }>;
      };
      taxInfo: {
        count: number;
        clients: Array<{ build: string; company_name: string }>;
  ...
  /**
   * Update agency credentials only (dedicated endpoint)
   */
  async updateAgencyCredentials(
    build: string,
    data: AgencyCredentials,
  ): Promise<void> {
    await api.patch(`/clients/${build}/agency-credentials`, data);
  },

  /**
   * Update DBD info only (dedicated endpoint)
   */
  async updateDbdInfo(build: string, data: DbdInfo): Promise<void> {
    await api.patch(`/clients/${build}/dbd-info`, data);
  },
