 ...
 */
async function fetchEmployeesBulk(employeeIds) {
 ...
 */
function derivePp30StatusFromRow(row) {
  if (!row) return null
...
/**
 * Helper function: สร้าง notification เมื่อผู้ตรวจบันทึกข้อมูลแล้ว
 ...
 */
async function createTaxInspectionCompletedNotification(
  employeeId,
  build,
  companyName,
  taxYear,
...
/**
 * Helper function: สร้าง notification เมื่อส่งรอตรวจ
 ...
 */
async function createTaxReviewNotification(
  employeeId,
  notificationType,
  taxType,
  build,
...
/**
 * Helper function: สร้าง notification เมื่อสถานะเปลี่ยนเป็น "ส่งลูกค้าแล้ว" (sent_to_customer)
 ...
 */
async function createSentToCustomerNotification(
  employeeId,
  taxType,
  build,
  companyName,
...
/**
 * Helper function: Mark notification as read when user views tax data
 * @param {string} monthlyTaxDataId - Monthly tax data ID
 * @param {string} userId - User ID ของผู้ที่เปิดดูข้อมูล
 */
async function markTaxReviewNotificationsAsRead(monthlyTaxDataId, userId) {
  try {
...
router.get('/:build/:year/:month', authenticateToken, async (req, res) => {
  try {
    ...
    // ถ้าไม่มี pp30_form → ใช้ derivePp30StatusFromRow เพื่อ derive จาก timestamp fields
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/05294dac-c144-4586-be72-5875c5682fcf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'backend/routes/monthly-tax-data.js:1141', message: 'H1: Raw pp30_form from DB', data: { build: taxData[0].build, pp30_form_raw: taxData[0].pp30_form, pp30_form_type: typeof taxData[0].pp30_form, pp30_form_is_null: taxData[0].pp30_form === null, pp30_form_is_undefined: taxData[0].pp30_form === undefined }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'H1' }) }).catch(() => { });
    // #endregion
    const pp30FormFromDb = taxData[0].pp30_form && String(taxData[0].pp30_form).trim() !== '' && taxData[0].pp30_form !== '0' && taxData[0].pp30_form !== '1' && taxData[0].pp30_form !== 0 && taxData[0].pp30_form !== 1
      ...
      : null
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/05294dac-c144-4586-be72-5875c5682fcf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'backend/routes/monthly-tax-data.js:1144', message: 'H1: pp30FormFromDb extracted', data: { build: taxData[0].build, pp30FormFromDb, pp30_form_raw: taxData[0].pp30_form }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'H1' }) }).catch(() => { });
    // #endregion
    const derivedStatus = derivePp30StatusFromRow(taxData[0])
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/05294dac-c144-4586-be72-5875c5682fcf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'backend/routes/monthly-tax-data.js:1147', message: 'H1: Derived status and final status', data: { build: taxData[0].build, pp30FormFromDb, derivedStatus, finalPp30Status: pp30FormFromDb || derivedStatus }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'H1' }) }).catch(() => { });
    // #endregion
    // ⚠️ สำคัญ: ใช้ pp30_form จากฐานข้อมูลก่อน (ให้ตรงกับที่บันทึกไว้)
    // ถ้าไม่มี pp30_form → ใช้ derivedStatus เป็น fallback
    const finalPp30Status = pp30FormFromDb || derivedStatus
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/05294dac-c144-4586-be72-5875c5682fcf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'backend/routes/monthly-tax-data.js:1174', message: 'H1: Response row before sending', data: { build: taxData[0].build, pp30_form: row.pp30_form, pp30FormFromDb, derivedStatus }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'H1' }) }).catch(() => { });
    // #endregion
...
router.post('/', authenticateToken, authorize('admin'), async (req, res) => {
  try {
    ...
    try {
      ...
      invalidateCache('GET:/api/monthly-tax-data/summary')
    } catch (cacheError) {
  ...
  try {
    ...
    if (actualParams.some(p => p === undefined)) {
      console.warn('⚠️ [Backend] Found undefined values in parameters! Converting to null...', {
        ...
        undefinedValues: actualParams.filter((p, i) => p === undefined).map((p, i) => ({ index: actualParams.indexOf(p), value: p })),
      })
    }
    ...
    try {
      ...
      if (responseData.build && responseData.tax_year && responseData.tax_month) {
        invalidateCache(`GET:/api/monthly-tax-data/${responseData.build}`)
      }
      // Invalidate by ID endpoint
      if (responseData.id) {
        invalidateCache(`GET:/api/monthly-tax-data/${responseData.id}`)
      }
    } catch (cacheError) {
      console.error('⚠️ Error invalidating cache (non-critical):', cacheError)
      // ไม่ throw error เพราะข้อมูลถูกบันทึกสำเร็จแล้ว
